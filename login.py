...
from dashboard import MainApp          # MainApp(conexion, login_win)
import pyodbc
...
class LoginApp(ctk.CTk):
    ...
    def crear_interfaz(self):
        try:
            ...
            img = ctk.CTkImage(light_image=Image.open("Img/login.png"),
                                size=(400, 400))
            ctk.CTkLabel(self.frame_izq, image=img, text="")\
                .place(relx=0, rely=0, relwidth=1, relheight=1)
        except Exception:
            ctk.CTkLabel(self.frame_izq, text="(imagen no encontrada)")\
                .pack(expand=True)
