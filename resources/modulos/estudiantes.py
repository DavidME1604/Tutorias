        ...
        try:
            with conexion.cursor() as cur:
                if nodo_actual == "fis" and not entry_facultad.get() == "FIS":
                    ...
                    cur.execute("EXEC spEliminarEstudiante ?, ?", (entry_codigo.get(), entry_facultad.get()))
            conexion.commit(); cargar_datos()
        except Exception as e:
    ...
    def consultar():
        try:
            with conexion.cursor() as cur:
                ...
                apellido  = entry_apellido.get().strip() or None
                correo    = entry_correo.get().strip() or None
                tutor     = entry_tutor.get().strip() or None
                carrera   = entry_carrera.get().strip() or None
                facultad  = entry_facultad.get().strip() or None

                # Ejecutar el procedimiento almacenado
                cur.execute("""
                    EXEC spBuscarEstudianteDinamico ?, ?, ?, ?, ?, ?, ?
                """, (codigo, nombre, apellido, correo, tutor, carrera, facultad))

                resultados = cur.fetchall()
                if not resultados:
                    messagebox.showinfo("Consulta vacía", "No se encontraron registros.")
                else:
                    for codigo, nombre, apellido, correo, tutor, carrera, facultad in resultados:
                        tabla.insert("", "end", values=(codigo, nombre, apellido, correo, tutor, carrera, facultad))
